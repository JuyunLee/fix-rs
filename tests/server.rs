...
#[macro_use]
mod common;
use common::{new_logon_message, TestStream, CLIENT_SENDER_COMP_ID, CLIENT_TARGET_COMP_ID};
...
use fix_rs::fixt;
use fix_rs::fixt::engine::{ConnectionTerminatedReason, EngineEvent};
...
fn test_wrong_target_comp_id_in_logon() {
    ...
    let (mut test_client, mut engine, _, connection) =
...
fn test_logon_all_fix_versions() {
    ...
    for fix_version in FIXVersion::all() {
        ...
        test_client.send_message_with_ver(
            fix_version,
            logon_message.default_appl_ver_id,
            logon_message,
        );
...
fn test_approve_already_approved_connection_does_nothing() {
    ...
    let (mut test_client, mut engine, _, connection) =
        TestStream::setup_test_client_and_logon(build_dictionary());
...
fn test_default_appl_ver_id() {
    ...
    //Connect and logon.
    let (mut test_client, mut engine, _, connection) =
        TestStream::setup_test_client_and_logon_with_ver(
            FIXVersion::FIXT_1_1,
...
fn test_block_read_while_approving_logon() {
    define_dictionary!(Heartbeat, Logon, TestRequest,);
...
fn test_auto_disconnect_after_no_logon() {
    ...
    let (test_client, mut engine, _, connection) =
        TestStream::setup_test_client(build_dictionary());
