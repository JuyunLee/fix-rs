...
use fix_rs::dictionary::fields::{
    AllocAccount, ClOrdID, EncryptMethod as EncryptMethodField, HeartBtInt, MsgSeqNum,
    NoMsgTypeGrp, OrigSendingTime, RateSource as RateSourceField,
    RateSourceType as RateSourceTypeField, RawData, RawDataLength,
    ReferencePage as ReferencePageField, SenderCompID, SendingTime, Symbol, TargetCompID,
    TestReqID, Text,
};
...
use fix_rs::message::{self, MessageDetails, NOT_REQUIRED, REQUIRED};
...
fn parse_message_with_ver<
    T: FIXTMessage + FIXTMessageBuildable + MessageDetails + Default + Any + Clone + PartialEq + Send,
>(
    fix_version: FIXVersion,
    message_version: MessageVersion,
    message: &[u8],
) -> Result<T, ParseError> {
    let mut message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>> =
        HashMap::new();
...
fn sender_comp_id_fourth_tag_test() {
    ...
    //FIX.4.0
    {
        ...
        let result = parse_message::<Heartbeat>(missing_sender_comp_id_tag_message);
        match result.err().unwrap() {
            fix_rs::fix::ParseError::MissingRequiredTag(tag, _) => {
                assert_eq!(tag, FieldTag(49));
            }
            _ => assert!(false),
        }
    }
...
fn appl_ver_id_sixth_tag_test() {
    ...
    parse_message_with_ver::<TestMessage>(
        FIXVersion::FIXT_1_1,
        MessageVersion::FIX50SP2,
        appl_ver_id_sixth_tag_message,
    )
    .unwrap();

    let appl_ver_id_fourth_tag_message =
        b"8=FIXT.1.1\x019=34\x0135=9999\x011128=9\x0156=CLIENT\x01112=Test\x0110=000\x01";
    ...
    parse_message_with_ver::<TestMessage>(
        FIXVersion::FIXT_1_1,
        MessageVersion::FIX50SP2,
        missing_appl_ver_id_tag_message,
    )
    .unwrap();
}
...
fn checksum_tag_test() {
    ...
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::NoValueAfterTag(tag) => {
            assert_eq!(tag, FieldTag(10));
        }
        _ => assert!(false),
    }
...
fn length_tag_test() {
    ...
    let valid_length_tag_message =
        b"8=FIX.4.2\x019=28\x0135=L\x0195=13\x0196=This\x01is=atest\x0110=130\x01";
    let message = parse_message::<LengthTagTestMessage>(valid_length_tag_message).unwrap();
    assert_eq!(
        message.meta.clone().unwrap().begin_string,
        FIXVersion::FIX_4_2
    );
    ...
    let missing_length_tag_message =
        b"8=FIX.4.2\x019=28\x0135=L\x0196=This\x01is=atest\x0110=190\x01";
    let result = parse_message::<LengthTagTestMessage>(missing_length_tag_message);
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::MissingPrecedingLengthTag(value_tag) => {
            assert_eq!(value_tag, FieldTag(96))
        }
        _ => assert!(false),
    }

    let late_length_tag_message =
        b"8=FIX.4.2\x019=28\x0135=L\x0196=This\x01is=atest\x0195=13\x0110=190\x01";
    ...
    let early_length_tag_message =
        b"8=FIX.4.2\x019=28\x0135=L\x0195=13\x0156=CLIENT\x0196=This\x01is=atest\x0110=190\x01";
    let result = parse_message::<LengthTagTestMessage>(early_length_tag_message);
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::MissingFollowingLengthTag(length_tag) => {
            assert_eq!(length_tag, FieldTag(95))
        }
        _ => assert!(false),
    }
}
...
fn repeating_groups_test() {
    ...
    define_message!(RateSourceGrp {
        REQUIRED,
        rate_source: RateSourceField[FIX40..],
        REQUIRED,
        rate_source_type: RateSourceTypeField[FIX40..],
        NOT_REQUIRED,
        reference_page: ReferencePageField[FIX40..],
    });
    ...
    let one_repeating_group_message =
        b"8=FIX.4.2\x019=26\x0135=L\x011445=1\x011446=0\x011447=0\x0110=168\x01";
    ...
    assert_eq!(
        message.rate_sources.first().unwrap().rate_source,
        RateSource::Bloomberg
    );
    assert_eq!(
        message.rate_sources.first().unwrap().rate_source_type,
    ...
    let message =
        parse_message::<RepeatingGroupsTestMessage>(one_repeating_group_with_optional_message)
            .unwrap();
    assert_eq!(message.rate_sources.len(), 1);
    assert_eq!(
        message.rate_sources.first().unwrap().rate_source,
    ...
    assert_eq!(
        message.rate_sources.first().unwrap().reference_page,
        b"SomeSource".to_vec()
    );
    ...
    let message =
        parse_message::<RepeatingGroupsTestMessage>(two_repeating_groups_message).unwrap();
    ...
    assert_eq!(
        message.rate_sources.get(1).unwrap().rate_source,
        RateSource::Reuters
    );
    assert_eq!(
        message.rate_sources.get(1).unwrap().rate_source_type,
        RateSourceType::Secondary
    );
    ...
    let message = parse_message::<RepeatingGroupsTestMessage>(
        two_repeating_groups_with_optional_first_message,
    )
    .unwrap();
    assert_eq!(message.rate_sources.len(), 2);
    assert_eq!(
    ...
    let message = parse_message::<RepeatingGroupsTestMessage>(
        two_repeating_groups_with_optional_second_message,
    )
    .unwrap();
    assert_eq!(message.rate_sources.len(), 2);
    assert_eq!(
    ...
    assert_eq!(
        message.rate_sources.get(1).unwrap().reference_page,
        b"SomeSource".to_vec()
    );
    ...
    let message =
        parse_message::<RepeatingGroupsTestMessage>(two_repeating_groups_not_body_end_message)
            .unwrap();
    assert_eq!(message.rate_sources.len(), 2);
    assert_eq!(
        message.rate_sources.first().unwrap().rate_source,
    ...
    let missing_one_repeating_group_message =
        b"8=FIX.4.2\x019=35\x0135=L\x011445=2\x011446=0\x011447=0\x0155=[N/A]\x0110=244\x01";
    let result = parse_message::<RepeatingGroupsTestMessage>(missing_one_repeating_group_message);
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::NonRepeatingGroupTagInRepeatingGroup(tag) => {
            assert_eq!(tag, FieldTag(55))
        }
        _ => assert!(false),
    }
    ...
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::RepeatingGroupTagWithNoRepeatingGroup(tag) => {
            assert_eq!(tag, FieldTag(1446))
        }
        _ => assert!(false),
    }
    ...
    let result = parse_message::<RepeatingGroupsTestMessage>(
        non_repeating_group_tag_in_repeating_group_message,
    );
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::NonRepeatingGroupTagInRepeatingGroup(tag) => {
    ...
    let result =
        parse_message::<RepeatingGroupsTestMessage>(wrong_first_tag_in_repeating_group_message);
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::MissingFirstRepeatingGroupTagAfterNumberOfRepeatingGroupTag(
            number_of_tag,
        ) => assert_eq!(number_of_tag, FieldTag(1445)),
        _ => assert!(false),
    }
    ...
    let result = parse_message::<RepeatingGroupsTestMessage>(
        wrong_first_tag_in_second_repeating_group_message,
    );
    assert!(result.is_err());
    match result.err().unwrap() {
    ...
    let missing_required_tag_in_repeating_group_message =
        b"8=FIX.4.2\x019=19\x0135=L\x011445=1\x011446=0\x0110=108\x01";
    let result = parse_message::<RepeatingGroupsTestMessage>(
        missing_required_tag_in_repeating_group_message,
    );
    assert!(result.is_err());
    match result.err().unwrap() {
    ...
    let missing_required_tag_in_first_repeating_group_message =
        b"8=FIX.4.2\x019=33\x0135=L\x011445=2\x011446=0\x011446=1\x011447=1\x0110=230\x01";
    let result = parse_message::<RepeatingGroupsTestMessage>(
        missing_required_tag_in_first_repeating_group_message,
    );
    assert!(result.is_err());
    match result.err().unwrap() {
        fix_rs::fix::ParseError::MissingRequiredTag(tag, _) => assert_eq!(tag, FieldTag(1447)),
...
fn nested_repeating_groups_test() {
    ...
    define_message!(Alloc {
        REQUIRED,
        alloc_account: AllocAccount[FIX40..],
    });

    define_message!(Order {
        REQUIRED,
        cl_ord_id: ClOrdID[FIX40..],
        NOT_REQUIRED,
        allocs: NoAllocs[FIX40..],
    });
    ...
    let one_nested_repeating_group_message =
        b"8=FIX.4.2\x019=35\x0135=L\x0173=1\x0111=uniqueid\x0178=1\x0179=acct\x0110=244\x01";
    let message =
        parse_message::<NestedRepeatingGroupsTestMessage>(one_nested_repeating_group_message)
            .unwrap();
    assert_eq!(
        message.meta.clone().unwrap().begin_string,
        FIXVersion::FIX_4_2
    ...
    assert_eq!(
        message.orders.first().unwrap().cl_ord_id,
        b"uniqueid".to_vec()
    );
...
fn stream_test() {
    ...
    for message in parser.messages {
        ...
        assert_eq!(
            casted_message.meta.clone().unwrap().begin_string,
            FIXVersion::FIX_4_2
        );
    ...
    let (bytes_read_success, result) =
        parser.parse(&invalid_message_before_valid_message[bytes_read_failure..].to_vec());
    assert!(result.is_ok());
    assert_eq!(
        bytes_read_failure + bytes_read_success,
        invalid_message_before_valid_message.len()
    );
    assert_eq!(parser.messages.len(), 1);
    let casted_message = parser
        .messages
...
fn equal_character_in_text_test() {
    let message =
        b"8=FIX.4.2\x019=37\x0135=L\x0134=177\x0198=0\x01108=30\x0158=some=text\x0110=176\x01";
...
fn no_value_after_tag_test() {
    let message =
        b"8=FIX.4.2\x019=37\x0135=L\x0134=\x0198=0\x01108=30\x0158=some=text\x0110=165\x01";
