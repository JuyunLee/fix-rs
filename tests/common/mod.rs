...
use fix_rs::fixt::engine::{Connection, Engine, EngineEvent, Listener};
...
fn estimate_body_length(message_bytes: &[u8]) -> usize {
    ...
    for (index, byte) in message_bytes.iter().enumerate() {
        ...
        if previous_byte == b'0'
            && *byte == b'='
            && message_bytes[index - 2] == b'1'
            && message_bytes[index - 3] == 1
        {
...
macro_rules! engine_poll_event {
    ( $engine:ident,$pat:pat => $body:expr ) => {{
        let result = $engine
            ...
            .expect("Engine does not have any events");
        if let $pat = result {
            $body
        } else {
...
impl TestStream {
    ...
    pub fn setup_test_server_with_ver(
        fix_version: FIXVersion,
        message_version: MessageVersion,
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Connection) {
        //Setup server listener socket.
        let addr = SocketAddr::V4(SocketAddrV4::new(
            Ipv4Addr::new(127, 0, 0, 1),
            SOCKET_PORT.fetch_add(1, Ordering::SeqCst) as u16,
        ));
        ...
        let connection = client
            .add_connection(
                fix_version,
                message_version,
                CLIENT_SENDER_COMP_ID,
                CLIENT_TARGET_COMP_ID,
                addr,
            )
            .unwrap();
        ...
        assert!(
            if let EngineEvent::ConnectionSucceeded(success_connection) = event {
    ...
    pub fn setup_test_server(
        ...
        Self::setup_test_server_with_ver(
            FIXVersion::FIXT_1_1,
    ...
    pub fn setup_test_server_and_logon_with_ver(
        fix_version: FIXVersion,
        message_version: MessageVersion,
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Connection) {
        ...
        let (mut test_server, mut client, connection) =
            Self::setup_test_server_with_ver(fix_version, message_version, message_dictionary);
        test_server
            .parser
            .set_default_message_version(MessageVersion::FIX50);
        ...
        client.send_message_box_with_message_version(
            connection,
            fix_version.max_message_version(),
        ...
        test_server
            .parser
            .set_default_message_version(message_version);

        (test_server, client, connection)
    }

    pub fn setup_test_server_and_logon(
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Connection) {
        Self::setup_test_server_and_logon_with_ver(
            FIXVersion::FIXT_1_1,
            MessageVersion::FIX50SP2,
            message_dictionary,
        )
    ...
    pub fn setup_test_client_with_ver(
        fix_version: FIXVersion,
        message_version: MessageVersion,
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Listener, Connection) {
        ...
        let mut client = Engine::new(message_dictionary.clone(), MAX_MESSAGE_SIZE).unwrap();
        let listener = client
            .add_listener(SERVER_SENDER_COMP_ID, &addr)
            .unwrap()
            .unwrap();
    ...
    pub fn setup_test_client(
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Listener, Connection) {
        Self::setup_test_client_with_ver(
            FIXVersion::FIXT_1_1,
            MessageVersion::FIX50SP2,
            message_dictionary,
        )
    ...
    pub fn setup_test_client_and_logon_with_ver(
        fix_version: FIXVersion,
        message_version: MessageVersion,
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Listener, Connection) {
        ...
        let (mut test_client, mut engine, listener, connection) =
            Self::setup_test_client_with_ver(fix_version, message_version, message_dictionary);
        test_client
            .parser
            .set_default_message_version(MessageVersion::FIX50);
        ...
        logon_message.default_appl_ver_id = message_version;
        test_client.send_message_with_ver(
            fix_version,
            fix_version.max_message_version(),
            logon_message,
        );
    ...
    pub fn setup_test_client_and_logon(
        message_dictionary: HashMap<&'static [u8], Box<BuildFIXTMessage + Send>>,
    ) -> (TestStream, Engine, Listener, Connection) {
        Self::setup_test_client_and_logon_with_ver(
            FIXVersion::FIXT_1_1,
            MessageVersion::FIX50SP2,
            message_dictionary,
        )
    ...
    pub fn recv_fixt_message(&mut self) -> Box<FIXTMessage + Send> {
        self.try_recv_fixt_message(Duration::from_secs(5))
            .expect("Did not receive FIXT message")
    }
    ...
    pub fn send_message_with_ver<T: FIXTMessage + Any + Send>(
        &mut self,
        fix_version: FIXVersion,
        message_version: MessageVersion,
        message: T,
    ...
    pub fn send_message_with_timeout<T: FIXTMessage + Any + Send>(
        &mut self,
        message: T,
        timeout: Duration,
    ) -> Result<(), usize> {
